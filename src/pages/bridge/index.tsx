...
export default function Bridge() {
  ...
  const handleSendAmount = useCallback(
    (sendAmount: string) => {
      ...
      if (tokenToBridge) {
        ...
        if (feeBch < 0.0001) feeBch = 0.0001
  ...
  const handleCurrencySelect = useCallback(
    async (currency: Currency) => {
      ...
      // setReceiveAmount('')
      if (currency) {
        const methodId = Object.values(anyswapInfo[chainFrom.id]).filter(
          (val: AvailableChainsInfo) =>
            Number(val.destChainID) === currency.chainId && val.symbol == currency.symbol && val.name == currency.name
        )[0].id
        ...
        if (shiftNeeded) {
          ...
          tokenTo.other.MinimumSwap = 0.01001
  ...
  const insufficientBalance = () => {
    if (hopDirection !== HopDirection.out) return false
  ...
  const buttonDisabled =
    maintenance || !currency0 || !sendAmount || sendAmount == '' || !aboveMin() || !belowMax() || insufficientBalance()

  const buttonText = maintenance
    ? i18n._(t`Bridge under temporary maintenance`)
    : !shiftAllowed
    ? i18n._(t`Bridge forbidden (blocked country)`)
    : !currency0
          ...
          <div className="p-4 space-y-4 rounded bg-dark-900" style={{ zIndex: 1 }}>
            ...
            <div className="flex justify-center p-4 text-center">
              <div className="flex items-center gap-1">
                <Typography component="h3" variant="base">
                  {i18n._(t`Bridge Bitcoin Cash to and from the SmartBCH Network`)}
                </Typography>
            ...
            {helpVisible && (
              <div className="p-3 mx-5 rounded bg-dark-800">
                ...
                <p className="py-4">
                  {i18n._(t`Bridge between BCH and SmartBCH or vice versa with hop.cash.`)}{' '}
                ...
                <p>
                  {i18n._(
                    t`For hop.cash related issues, note the BCH and SBCH transaction identifiers and request assistance here:`
                  )}{' '}
            ...
            <div className={classNames('mt-0 pt-0 pb-0 rounded bg-dark-800')} style={{ margin: 0 }}>
              ...
              {hopDirection === HopDirection.out && (
                <div className={classNames('mt-0 p-5 pt-0 rounded rounded-t-none bg-dark-800')} style={{ margin: 0 }}>
                  <div
                    className={
                      'flex flex-col items-center md:text-xl text-base justify-between space-y-3 sm:space-y-0 sm:flex-row'
                    }
                  >
                    <div className={classNames('w-full sm:w-72')}>{i18n._(t`Destination address`)}</div>
                    <div
              ...
              <div className={'p-2 sm:p-5 rounded bg-dark-800'}>
                {tokenToBridge?.other?.MinimumSwapFee > 0 && (
                  <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                    <div className="text-sm font-medium text-secondary">
                      {i18n._(
                        t`Minimum Bridge Fee: ${formatNumber(tokenToBridge?.other?.MinimumSwapFee)} ${
                ...
                {tokenToBridge?.other?.MaximumSwapFee > 0 && (
                  <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                    <div className="text-sm font-medium text-secondary">
                      {i18n._(
                        t`Maximum Bridge Fee: ${formatNumber(tokenToBridge?.other?.MaximumSwapFee)} ${
                          tokenToBridge?.other?.Symbol
                        }`
                      )}
                    </div>
                ...
                <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
                  <div className="text-sm font-medium text-secondary">
                    {i18n._(
                      t`Estimated Fee Included: ${swapInfo?.feeUsd.toFixed(2)} USD + ${swapInfo?.feeBch.toFixed(5)} BCH`
                    )}
                  </div>
                </div>
                <div className="flex flex-col justify-between space-y-3 sm:space-y-0 sm:flex-row">
