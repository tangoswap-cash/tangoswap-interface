...
const needsDots = (message: string) => {
  if (
    message.includes(i18n._(t`Bridge process cancelled due to error:`)) ||
    message.includes(i18n._(t`Funds arrived to destination`))
  ) {
    return false
  }
  return true
...
export default function BridgeModal({ isOpen, hash, onDismiss }: BridgeModalProps) {
  ...
  const address =
    (bridgeTransaction.shiftStatus || {}).depositAddress || (bridgeTransaction.hopStatus || {}).depositAddress || null
  ...
  const [depositAddress, setDepositAddress] = useState<string | null>(
    bridgeTransaction.hopStatus?.direction === HopDirection.in ? address : null
  )
  ...
  const depositAddressWithAmount = shiftNeeded
    ? depositAddress
    : `${depositAddress}?amount=${bridgeTransaction.initialAmount}`

  useEffect(() => {
    const stateMachine = async () => {
      if (isOpen && transactionFound) {
        const hopProcess =
          bridgeTransaction.hopStatus?.direction === HopDirection.in
        ...
        if (
          (hopProcess.stage != HopStage.settled && hopProcess.stage != HopStage.cancelled) ||
        ...
        <Modal isOpen={isOpen} onDismiss={onClose}>
          <div className="space-y-4">
            ...
            <div className="items-center justify-center gap-2 sm:flex">
              ...
              <Copy toCopy={hash}>{shorten(hash, 8)}</Copy>
            </div>
            ...
            {destinationTag && (
              <div className="flex items-center justify-center">
                <Typography className="font-medium" variant="sm">
                  {i18n._(
                    t`Your XRP deposit must contain the Destination Tag: "${destinationTag}", otherwise the deposit will be rejected by the network`
                  )}
                </Typography>
              </div>
            )}
              ...
              style={
                {
                  display: 'flex',
                  'flex-direction':
                    bridgeTransaction.hopStatus.direction === HopDirection.in ? 'column' : 'column-reverse',
                } as any
              }
            >
              {sideShiftOrderId && (
                <div>
                  <div className="flex items-center justify-center">
                    <a
                      href={`https://sideshift.ai/orders/${sideShiftOrderId}`}
                      target="_blank"
              ...
              {bchTransactionId && (
                <div>
                  <div className="flex items-center justify-center">
                    <a
                      href={`https://blockchair.com/bitcoin-cash/transaction/${bchTransactionId}`}
                      target="_blank"
                      className="flex flex-row items-center gap-1 font-bold text-baseline text-primary"
                      rel="noreferrer"
                    >
                      {i18n._(t`BCH cross-chain tx ${shorten(bchTransactionId)}`)}{' '}
              ...
              {sbchTransactionId && (
                <div>
                  <div className="flex items-center justify-center">
                    <a
                      href={`https://www.sonar.cash/tx/${sbchTransactionId}`}
                      target="_blank"
                      className="flex flex-row items-center gap-1 font-bold text-baseline text-primary"
                      rel="noreferrer"
                    >
                      {i18n._(t`SmartBCH cross-chain tx ${shorten(sbchTransactionId)}`)}{' '}
                      <ExternalLinkIcon width={20} height={20} />
                    </a>
                  </div>
                </div>
            ...
            {statusText && (
              <div className="flex items-center justify-center gap-2">
                {needsDots(statusText) && <Dots>{statusText}</Dots>}
